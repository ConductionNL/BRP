...
The Helm chart installs brpservice and by default the following dependencies using subcharts:
...
```bash
helm repo add brpservice https://raw.githubusercontent.com/ConductionNL/brpservice/master/api/helm/
helm repo update
```
...
```bash
helm install my-brpservice ConductionNL/brpservice --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `brpservice` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `brpservice` |
